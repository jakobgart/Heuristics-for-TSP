		...
		for(int it = 0 ; it < numIter ; it++){
			...
			for(int a = 0 ; a < nCity-1 ; a++){
				...
				for(int i = 0 ; i < numAnts; i++){
					...
					//we have to reduce next by 1 since its actual citiy -> index conversion
					next--;
					
					//the loop above will stop with the next city to visit for ant i
					//now we just add the city to the ant
			...
			//we'll calculate the added pheromone for each arc and set the new pheromone
				...
				for(int y = x+1 ; y < nCity ; y++){
					...
					for(int i=0 ; i < numAnts ; i++){
						...
						if(ants[i].hasTransition(x+1,y+1)){
							//System.out.println(String.format("%.1f/%.1f=%.10f is how much we add to connection %d - %d", Q, ants[i].weight,(Q/ants[i].weight), x+1, y+1));REMOVE
							addPher+=(Q/ants[i].weight);
						}
					}
					/*//REMOVE
					if(addPher>0.002){
						System.out.println(String.format("we're adding %.1f extra to the connection %d - %d", addPher, x+1, y+1));
					}*/
			...
			//System.out.println("Current best in iteration is: " + curBest.weight);
