...
public class Solution {
	...
	*/
	...
	/*
	reverses the cities and all the cities between them (including the two cities).
	
	example:
	TSP =	[1 2 3 4 5 6 7 8]
	c1 =	2
	c2 =	8
	output:	[1 8 7 6 5 4 3 2]
	*/
	public void reverseCities(int c1, int c2){
		
		int temp;
		
		//make sure they're oredered properly
		if(this.getIndexOfCity(c1)>this.getIndexOfCity(c2)){
			temp=c1;
			c1=c2;
			c2=temp;
		}
		...
		//length of array
		int len=this.getIndexOfCity(c2)-this.getIndexOfCity(c1)+1;
		
		//position of c1
		int c1Pos = this.getIndexOfCity(c1);
		...
		//fill up the array and update city
		for(int i = 0 ; i < len ; i++){
			rev[i]=this.tour[c1Pos+len-1-i];
			city[this.tour[c1Pos+len-1-i]-1]=c1Pos+i+1;
		}
		
		//replace the array
		for(int i = 0 ; i < len ; i++){
			this.tour[c1Pos+i]=rev[i];
		}
